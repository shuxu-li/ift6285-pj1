...
def process_folder(folder_name):
    ...
    # prediction_bert.to_csv(f'{folder_name}/prediction_bert.csv', index=False)
    # print(f"distilbert_svm: {folder_name} {accuracy_score(test_labels, predict_bert_svm)}" )


    # Count Vectorizer + SVM
    
    model_count = Pipeline([("vectorizer", CountVectorizer(ngram_range=(1, 2))), ("classifier", SVC())])
    ...
    prediction_count.to_csv(f'sortie/{folder_name}-count_svc.csv', index=False)
